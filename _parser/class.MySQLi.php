<?php
	...
	class dbMySQLi {
		...
		function __construct($server, $username = null, $password = null, $database = null, $enc = false) {
			$this->db_data = array();
			$this->db_data['server'] = $server;
			$this->db_data['username'] = $username;
			$this->db_data['password'] = $password;
			$this->db_data['database'] = $database;
			$this->db_data['enc'] = $enc;
			$this->Connect();
		}
		...
		function Connect() {
			...
			$ts = explode(':', $this->db_data['server']);
			if(!@$ts[1]) {
				$this->_db = mysqli_connect($this->db_data['server'], $this->db_data['username'], $this->db_data['password'], $this->db_data['database']);
			} else {
				$this->_db = mysqli_connect($ts[0], $this->db_data['username'], $this->db_data['password'], $this->db_data['database'], $ts[1]);
			}
			if(!$this->_db) {
				$this->_error(__METHOD__);
				return false;
			}
			if($this->db_data['enc'])  $this->Query('SET NAMES '.$this->db_data['enc']);
		}
		...
		function GetOrCreate($table, $idname, $wheres, $params=array()) {
			$wheress = array();
			foreach($wheres as $n=>$v) {
				$wheress[] = '`'.$n.'`='.$this->Escape($v);
				$params[$n] = $v;
			}
			$wheress = '('.implode(') AND (', $wheress).')';
			$ret = $this->Fetch1('SELECT '.$idname.' FROM '.$table.' WHERE '.$wheress);
			if(!$ret) {
