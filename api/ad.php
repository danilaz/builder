...
$id   = $_GET['id'];
if(!$cache->begin(array($id,$spid,$scid,$dpid,$dcid),3600*0))
{
	$sub_sql=$tsql=NULL;$nt=date("Y-m-d");
	if(!empty($dpid))
		$sub_sql=" and province='$dpid' and city=''";
	if(!empty($dcid))
		$sub_sql=" and city='$dcid' ";
	if(empty($dcid)&&empty($dpid))
		$sub_sql=" and province='' and city='' ";
	if(!empty($_GET['catid']))
		$tsql.=" and catid='$_GET[catid]'";
	if(!empty($_GET['sname']))
		$tsql.=" and sname='$_GET[sname]'";	
	
	$db->query("update ".ADVSCON." set shownum=shownum+1 where group_id='$id'");
	$db->query("select ad_type,width,height from ".ADVS." where ID='$id'");
	$ad_re=$db->fetchRow();
	if($ad_re['ad_type']==2)
	{//flash幻灯片
		$sql="select * from ".ADVSCON." where group_id='$id' and isopen=1 and etime>='$nt' and stime<='$nt' and type=3 $sub_sql $tsql ";
		$db->query($sql);
		$re=$db->getRows();
		if(count($re)<=0)
		{
			$sql="select * from ".ADVSCON." where 
			group_id='$id' and isopen=1 and etime>='$nt' and stime<='$nt' $sub_sql and (sname='' or sname is NULL) and (catid='' or catid is NULL)";
			$db->query($sql);
			$re=$db->getRows();
		}
	...
	{//对联广告
		$sql="select * from ".ADVSCON." where group_id='$id' and isopen=1 and etime>='$nt' and stime<='$nt' $sub_sql $tsql ";
		$db->query($sql);
		$re=$db->getRows();
		if(count($re)<=0)
		{
