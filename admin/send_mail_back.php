...
<?php
		$sql="select a.user,a.lastLoginTime,a.email,b.contact,b.company from ".ALLUSER." a, ".USER." b where 
		a.userid=b.userid and a.email<>'' $sqlw order by a.userid desc limit ".$limits.",5";
		$limits=$limits+5;
	    $db->query($sql);
	    $re=$db->getRows();
        if ($db->num_rows()>0)
