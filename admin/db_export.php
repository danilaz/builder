...
if($_GET['del'])
{
	...
	admin_msg('db_export.php',"Успешно удалено!"); 
}
if($_GET['t'] and $_GET['num'])
{
	if (!@mysql_connect($config['dbhost'],$config['dbuser'],$config['dbpass']))
	{
		echo "'.lang_show('can_not_con_server').'";exit;
	}
	if(!@mysql_select_db($config['dbname'])) 
	{
		echo "'.lang_show('db_is_not_exist').'";exit;
	}
	...
	if(file_exists($b2bbuilder_backup=$config['webroot'].$backup_base."/".$_GET['t']."/".$_GET['t']."_".$_GET['num'].".php"))
	{
		...
		{
			echo $_GET['t']."_".$_GET['num'].".php Резервное копирование выполнено!";
...
if(isset($_POST['submit']))
{
	...
	admin_msg('db_export.php',"Резервное копирование выполнено!"); 
}
	...
	<div class="bigboxbody">
        <table width="100%" border="0" cellspacing="0" cellpadding="0" >
            <form action="" method="post">
                <tr>
                <td>
                <input class="btn" type="submit" name="submit" value="<?php echo lang_show('backup');?>" >
                </td>
                </tr>
            </form>
        </table>
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0" >
		  <?php foreach($arr as $key=>$val){ ?> 
           <tr>
                ...
                <td><a  href="db_export.php?num=1&t=<?php echo $val['name']; ?>">Восстановить</a></td>
                <td><a  href="db_export.php?del=<?php echo $val['name']; ?>">Удалить</a></td>
           </tr>
           <?php } ?>
        </table>
        <?php } ?>
