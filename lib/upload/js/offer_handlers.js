	...
	try {
		...
		case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
			try {
				...
				progress.setStatus("<font color='red'>Файл не может быть больше 2 Мб!</font>");
...
function addImage(src,file,name,size) {
	...
	for(var i=0;i<nails.length;i++)
	{
		...
		if(addStatu==0){
			...
			filename=filename.length>10?filename.substring(0,3)+'...'+filename.substring(filename.lastIndexOf('.')-2):filename;
			nameBox[0].innerHTML= filename;
			var commendBtn = document.createElement('a');
			commendBtn.href='javascript:void(0);';
			commendBtn.appendChild(document.createTextNode('Удалить'));
