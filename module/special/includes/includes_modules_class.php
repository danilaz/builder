...
class includes_modules
{
	...
	function special_list($va)
	{
		...
		$sql="select * from ".SPE." where 1 order by `order` asc limit 0,$va[nums]";//×¨ÌâÏêÇé
		$this->db->query($sql);
	...
	function news_list($va)
	{
		...
		{
			if($user==true)
				$va['list'][$key]['url']= geturl('newsd',$val['uid'],$val['auther'],$val['nid']);
			else
			{
	...
	function offer_list($va)
	{
		$va=$this->stripsla(unserialize($va));
		if($va['type']==1)
			$sql="select * from ".INFO." where 1 order by id desc limit 0,$va[nums]";
		if($va['type']==2)
			$sql="select * from ".INFO." where 1 and statu=1 order by id desc limit 0,$va[nums]";
		if($va['type']==3)
			$sql="select * from ".INFO." where 1 and id in ($va[nums]) order by id desc";
		if($va['type']==4)
			$sql=$va['sql'];
