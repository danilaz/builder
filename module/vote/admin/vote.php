...
if($_POST['act'] == 'add')
{
	...
	$sql="INSERT ".NEWSVOTE." (title,votetext,votetype,num,limitip,time,uptime) VALUES('$_POST[vname]','$votetext','$_POST[vtype]','$count','$_POST[vlimit]','$_POST[vtime]',".time().")";
	$re=$db->query($sql);
...
if($_POST['act']== 'edit' and !empty($_GET['vid']))
{
	...
	$sql="update ".NEWSVOTE."  set title='$_POST[vname]',votetext='$votetext',votetype='$_POST[vtype]',num='$count',limitip='$_POST[vlimit]',time='$_POST[vtime]' where id=".$_GET['vid'];
	$re=$db->query($sql);
	msg("module.php?m=vote&s=vote_list.php");
}
...
<style>
.bigboxbody{ border:none;}
.bigboxbody .td{ padding-left:5px; border:0px; background-color:#f0f0f0}
...
<form action="" method="post">
	...
	<tr class="tr">
	   ...
	   <td class="td">
	       ...
	       <table cellpadding="0" cellspacing="0" width="80%" border="0" class="vote" id="vote">
		      <tr class="tr">
			     <td class="td" width="8%" align="center">Номер</td><td width="30%" class="td">Название предложения</td><td class="td">Голосов</td>
			  </tr>
			  <?php
			    if(!empty($vote))
				{  
             ...
             <?php
			  ...
			  ?>
              <tr class="tr">
              	<td class="td" colspan="2">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" value="Добавить строку" onclick="add('vote')" /></td>
              	<td class="td">Всего голосов: <?php if(!empty($re['num'])) echo $re['num']; else echo '0'; ?></td>
              </tr>  
		   </table>
        </td>
	</tr>
	...
	<tr class="tr">
	   <td class="td"></td>
	   <td class="td"><input type="submit" name="submit" id="submit" value="<?php if(!isset($_GET['vid'])) echo lang_show('addvote'); else  echo lang_show('editvote'); ?>">
