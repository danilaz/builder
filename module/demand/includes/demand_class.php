...
class demand
{
	...
	function add_info()
	{
		...
		$sql="insert into ".$config['table_pre']."demand"."
		...
		$this->db->query($sql);
		$id=$this->db->lastid();
		$this->db->query("INSERT INTO ".DEMANDDETAIL." (userid,demandid,detail) values ('$buid','$id','$_POST[detail]')");
		
		update_cat_nums($catid,'add','demand');
		//##======================
		if($_POST['custom_cat'])
		{
		...
		if($point_config['point']=='1'&&$point_config['add_demand']!='0')
			renew_point('',$point_config['add_demand']);
		return $id;
	}
	function edit_info()
	{
		...
		if(!empty($_POST['sscatid']))
			$catid=$_POST['sscatid'];			
		$cpc=$this->get_user_pv($buid);
		$sql="update ".$config['table_pre']."demand"." SET 
		...
		{	
			...
			//##==========================
			$this->db->query("update ".DEMANDDETAIL." set detail='$_POST[detail]' where demandid='$_POST[editID]'");
			include_once($config['webroot']."/includes/tag_inc.php");
	...
	{
		global $buid,$config;
		$sql="select id,type,title,uptime,statu from ".$config['table_pre']."demand"." where userid='$buid' order by uptime desc";
		//=============================
	  	$page = new Page;
		$page->listRows=20;
		if (!$page->__get('totalRows')){
			$this->db->query("select count(id) as num from ".$config['table_pre']."demand"." where userid='$buid'");
	...
	function space_demand_list($catid)
	{
		global $config;
		if(!empty($catid))
		{	
			$sql="select * from ".$config['table_pre']."demand"." a, ".CUSTOM_CAT_REL." b 
				 where a.userid='$_GET[uid]' and a.statu>0 and a.id=b.relating_id and b.custom_cat_type=2 and b.custom_cat_id='$catid'
				 order by uptime desc";
		}
		else
		{
			$sql="select id,`type`,title,con,uptime,userid,user,pic from ".$config['table_pre']."demand"." 
	...
	function del_info($id)
	{
		...
		include($config['webroot']."/config/point_config.php");
		if($point_config['point']=='1'&&$point_config['del_demand']!='0')
			renew_point('',$point_config['del_demand']);
		
		$this->db->query("select catid from ".$config['table_pre']."demand"." where id='$id'");
		$re=$this->db->fetchRow();
		update_cat_nums($re['catid'],'del','demand');

		$this->db->query("delete from  ".$config['table_pre']."demand"." where id='$id' and userid='$buid'");
		$this->db->query("delete from  ".DEMANDDETAIL." where demandid='$id'");
	...
	function update_info($id)
	{
		...
		if($id=="all")
		{
			...
			if($point_config['point']=='1'&&$point_config['renew_demand']!='0')
			renew_point('',$point_config['renew_demand']);
			$sql="update ".$config['table_pre']."demand"." SET uptime='$this->cTime' where userid='$buid'";
		}
		else
			$sql="update ".$config['table_pre']."demand"." SET uptime='$this->cTime' where id='$id'";
		$this->db->query($sql);
	}
	...
	function info_detail($id)
	{
		if($id)
		{
			global $config,$buid;
			include_once($config['webroot']."/includes/tag_inc.php");
			$sql="select a.*,b.detail from ".$config['table_pre']."demand"." a left join ".DEMANDDETAIL." b on a.id=b.demandid where a.id='$id' and b.userid='$buid'";
			$this->db->query($sql);
			$re=$this->db->fetchRow();
	...
	//--------------------------------------
	function company_other_demand($uid,$type=NULL,$limit=NULL)
	{
    global $config;
		if(!empty($limit))
			$sub_sql="limit 0,$limit";
		$sql="select title,id,type from ".$config['table_pre']."demand"." where userid='$uid' $sub_sql";
		$this->db->query($sql);
		return $this->db->getRows();
	}
	//========================
	function same_cat_demand($catid)
	{
		global $config;
		if(!empty($_POST['province']))
			$subsql=" and a.province='$_POST[province]'";

		$sql="select * from ".$config['table_pre']."demand"." a left join ".USER." b on a.userid=b.userid
		 		where 
	...
	function shop_info_detail($id)
	{
		...
		if(is_numeric($id))
		{
			...
			$sql="select a.*,b.detail from ".$config['table_pre']."demand"." a left join ".DEMANDDETAIL." b on a.id=b.demandid 
			      where a.id='$_GET[id]'";
			$this->db->query($sql);
			...
			//##=========================
			$info['demandtype']=$infoType[$info['type']];
			if(!empty($info['pic']))
	...
	function demand_merge_user($array)
	{
		global $config;
		$old_uid=$array['old_uid'];
		$new_uid=$array['new_uid'];

		$sql = "update ".$config['table_pre']."demand"." set userid='$new_uid' where userid='$old_uid'";
		$this->db->query($sql);
		$sql = "update ".DEMANDDETAIL." set userid='$new_uid' where userid='$old_uid'";
		$this->db->query($sql);
	}
}
?>
