...
if(!empty($_POST["cc"])&&$_POST["cc"]==lang_show('modify'))
{
	...
	$db->query($sql);
	$db->query("update ".DEMANDDETAIL." set detail='$_POST[detail]' where demandid='$_GET[id]'");
	edit_tags($_POST['keyword'],2,$_GET['id']);
	
	unset($_GET['id']);unset($_GET['s']);
	$getstr=implode('&',convert($_GET));
	msg("module.php?s=demandlist.php&$getstr");
}
$sql="select a.*,b.detail from ".$config['table_pre']."demand"." a left join ".DEMANDDETAIL." b on a.id=b.demandid where a.id='$_GET[id]'";
$db->query($sql);
if($db->next_record())
{
	$id=$db->f('id');
