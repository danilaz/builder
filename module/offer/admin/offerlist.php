...
if(!empty($_GET["submit"])&&$_GET["submit"]==lang_show('delete'))
{
	if(isset($_GET["de"]) && is_array($_GET["de"]))	{
		...
		{
			$db->query("select catid from ".INFO." where id='$v'");
			$re=$db->fetchRow();
		...
		$db->query("delete from  ".OFFERDETAIL." where offerid in ($id)");
...
if(!empty($_GET["submit"])&&$_GET["submit"]==lang_show('auditpass')){
	if(isset($_GET["de"]) && is_array($_GET["de"])){
		$id=implode(",",$_GET["de"]);
		$sql="update ".INFO." set statu=1 where id in ($id)";
		$db->query($sql);
	}
}
if(!empty($_GET["submit"])&&$_GET["submit"]==lang_show('rc')){
	if(isset($_GET["de"]) && is_array($_GET["de"])){
		$id=implode(",",$_GET["de"]);
		$sql="update ".INFO." set statu=2 where id in ($id)";
		$db->query($sql);
	}
}
if(!empty($_GET["submit"])&&$_GET["submit"]==lang_show('notpass')){
	if(isset($_GET["de"]) && is_array($_GET["de"])){
		$id=implode(",",$_GET["de"]);
		$sql="update ".INFO." set statu=0 where id in ($id)";
		$db->query($sql);
	}
}
if(!empty($_GET["submit"])&&$_GET["submit"]==lang_show('unrc')){
	if(isset($_GET["de"]) && is_array($_GET["de"])){
		$id=implode(",",$_GET["de"]);
		$sql="update ".INFO." set statu=1 where id in ($id) and statu>1";
		$db->query($sql);
	}
}
if(!empty($_GET["submit"])&&$_GET["submit"]==lang_show('okeyalli')){
	$sql="update ".INFO." set statu=1 where statu=0";
	$db->query($sql);
}
...
<div class="bigbox">
  ...
  <tr>
	...
	<input name="m" type="hidden" value="offer">
	<input name="s" type="hidden" value="offerlist.php">
	</td>
  </tr>
</table>
...
<div style="float:left; height:50px; width:50%;">&nbsp;</div>
  ...
  <table width="100%" border="0" cellpadding="2" cellspacing="0" >
	...
	from ".INFO." as a left join ".USER." as b on a.userid=b.userid  where 1 $scl order by id desc ";
	//=============================
	$page = new Page;
	$page->listRows=20;
	...
	?>
		...
		<select  style="visibility:hidden" name="catid<?php echo $i?>" id="catid<?php echo $i?>" onChange="getHTML(this.value,'tcatid<?php echo $i?>',<?php echo $i?>,'offer')">
	...
	<?php		}
		...
		<select style="visibility:hidden" name="tcatid<?php echo $i?>" id="tcatid<?php echo $i?>" onChange="getHTML(this.value,'scatid<?php echo $i?>',<?php echo $i?>,'offer')">
		...
		<span id="mod<?php echo $i?>" style="visibility:hidden"><input type="button" value="<?php echo lang_show('mod');?>" onClick="updateCat('offer',<?php echo $v['id']; ?>,<?php echo $i?>);">	  </td>
        ...
        <a href="module.php?m=offer&s=offermod.php&id=<?php echo $v['id']."&$getstr"; ?>"><?php echo lang_show('manager');?></a>		</td>
      </tr>
    <?php 
		$i++;
    }
