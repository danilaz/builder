...
if (!empty($_GET['cid']))
{
	...
	if ($db->num_rows()>0)
	{
       ...
       foreach($rs as $v)
       {
           ...
           echo "$i.<a href='?action=demand_detail&demandid=".$v['id']."'>".$v['title']."</a><><br/>";
		   $i=$i+1;
       }
	   echo "<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='?action=demand_cat'><i>Вернуться</i></a><br/>";
...
{
	header("Location:./?action=demand_list&cid=".$cid);
	exit();
}
?>